...
struct CompanyHistoryView: View {
    @ObservedObject var mViewModel: CompanyViewModel
    var mEntityId: Int
    
    init(entityId: Int) {
        self.mEntityId = entityId
        self.mViewModel = CompanyViewModel(entityId: entityId, include: "history")
    }
            ...
            if self.mViewModel.IsLoading == true  {
                ...
                List {
                    if self.mViewModel.Company != nil {
                        ForEach(self.mViewModel.Company!.History) { history in
                            VStack {
                                VStack {
                                    HStack {
                                        HStack {
