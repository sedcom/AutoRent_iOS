    ...
    var body: some View {
        VStack {
            if self.mIndex < self.mViewModel.Application!.Items.count {
                VStack {
                    VStack {
                        CustomText(String(format: NSLocalizedString("string_applicationitem_title", comment: ""), String(self.mIndex + 1)), alignment: .trailing, maxLines: 1, bold: true, selected: self.mIsSelected)
                    }
                    HStack {
                        HStack {
                             Image("calendar-alt")
                             ...
                             VStack {
                                 CustomText(Utils.formatDate(format: "dd MMMM yyyy", date: self.mViewModel.Application!.Items[self.mIndex].StartDate), alignment: .center, maxLines: 1, selected: true)
                                ...
                                CustomText(Utils.formatDate(format: "HH:mm ZZZZZ", date: self.mViewModel.Application!.Items[self.mIndex].StartDate), alignment: .center, maxLines: 1, bold: true, selected: true)
                             }
                         }
                        ...
                        HStack {
                            Image("calendar-alt")
                                ...
                                .foregroundColor(Color.textDark)
                            VStack {
                                CustomText(Utils.formatDate(format: "dd MMMM yyyy", date: self.mViewModel.Application!.Items[self.mIndex].FinishDate), alignment: .center, maxLines: 1, selected: true)
                                .onChange(of:  $SelectedDate.wrappedValue, perform: { value in
                                ...
                                CustomText(Utils.formatDate(format: "HH:mm ZZZZZ", date: self.mViewModel.Application!.Items[self.mIndex].FinishDate), alignment: .center, maxLines: 1, bold: true, selected: true)
                             }
                         }
                        .padding(EdgeInsets(top: 12, leading: 8, bottom: 12, trailing: 8))
                        .frame(minWidth: 0, maxWidth: .infinity)
